...
RSpec.describe CsvConverter, type: :model do
  ...
  describe '#convert' do
    before do
      user = User.create(email: 'test@example.com', user_name: 'name',
                         first_name: 'first', last_name: 'last',
                         password: 'password')
      feature = FactoryBot.create(:feature, user: user)

      FactoryBot.create(:mapping, user_column: 'item_number', ec_column: 'handle', data_type: 0, feature: feature)
      FactoryBot.create(:mapping, user_column: 'item_name', ec_column: 'title', data_type: 0, feature: feature)
      FactoryBot.create(:mapping, user_column: 'price', ec_column: 'price', data_type: 1, feature: feature)
    end
    ...
    context '正しいinputファイルが渡された時' do
      before do
        converter = CsvConverter.new(input_file_path, mapping)
    ...
    context 'inputファイルが空の場合' do
      before do
        converter = CsvConverter.new(empty_file_path, mapping)
        converter.convert_csv(output_file_path)
        @output = CSV.read(output_file_path, headers: true)
      end
